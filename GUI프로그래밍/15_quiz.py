...
def open_file():
    if os.path.isfile(filename): # 파일 있으면 True, 없으면 False // import os
...
menu_file = Menu(menu, tearoff=0) # tearoff ? // 
...
# 본문 영역
txt = Text(root, yscrollcommand=scrollbar.set) # 스크롤바 매핑1
txt.pack(side="left", fill="both", expand=True) # fill="both", expand=True

scrollbar.config(command=txt.yview) # 스크롤바 매핑2 

root.config(menu=menu) # ★ 꼭 해줘야 한다.
root.mainloop()